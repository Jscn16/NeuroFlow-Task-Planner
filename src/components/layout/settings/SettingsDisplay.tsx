...
export const SettingsDisplay: React.FC<SettingsDisplayProps> = ({
    ...
    const { t } = useLanguage();
    return (
        <SettingsSection title={t.settings.nightOwl} icon={Moon} defaultOpen={false}>
            <p className="text-[11px] mb-3" style={{ color: 'var(--text-muted)' }}>
                {t.settings.nightOwlSubtitle}
            </p>

            <div className="space-y-3">
                {/* Toggle */}
                <div className="flex items-center justify-between">
                    <span className="text-sm font-medium" style={{ color: 'var(--text-primary)' }}>{t.settings.nightOwlLabel}</span>
                    <button
                ...
                {dayBoundary !== 0 && (
                    <div className="pt-2 flex items-center justify-between animate-in slide-in-from-top-2 fade-in duration-200">
                        <span className="text-xs" style={{ color: 'var(--text-secondary)' }}>{t.settings.nightOwlTime}</span>
